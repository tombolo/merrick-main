...
export default class SaveModalStore implements ISaveModalStore {
    ...
    addStrategyToWorkspace = async (
        ...
        try {
            ...
            const save_collection = save_as_collection ? save_types.UNSAVED : save_types.GOOGLE_DRIVE;
    ...
    onConfirmSave = async ({ save_as_collection, bot_name }: IOnConfirmProps) => {
        ...
        if (active_tab === 1) {
            ...
            main_strategy.save_type = save_types.GOOGLE_DRIVE; // Always use Google Drive
        ...
        try {
            ...
            if (active_tab === 0) {
                ...
                await this.addStrategyToWorkspace(workspace_id, false, save_as_collection, bot_name, xml);
                if (main_strategy) await loadStrategyToBuilder(main_strategy);
            } else {
                await saveWorkspaceToRecent(xml, save_types.GOOGLE_DRIVE);
            }
        } catch (error) {
