...
export default class JournalStore {
    ...
    pushMessage(
        ...
        message_type: string,
        className?: string,
        extra: { current_currency?: string; currency?: string; profit?: number } = {}
    ) {
        ...
        if (is_special_demo && typeof message === 'string' && extra?.profit !== undefined) {
            // If it's a loss, convert it to show potential profit instead
            if (extra.profit < 0) {
                const potential_profit = extra.profit * -1; // Convert loss to positive
                message = message.replace(/[-]?[\d.,]+/, potential_profit.toString());
                extra.profit = potential_profit;
            }
        }
